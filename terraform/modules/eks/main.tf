# EKS Module - Kubernetes Cluster for NDMarket Application
...
# Bedrock Access Policy for Node Group (AI features)
resource "aws_iam_role_policy" "eks_node_bedrock_access" {
  name = "${var.project_name}-${var.environment}-eks-node-bedrock-policy"
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:bedrock:${var.region}::foundation-model/amazon.titan-embed-text-*"
        ]
      }
    ]
  })
...
resource "aws_security_group" "eks_nodes" {
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.eks_cluster.id]
    description     = "Allow cluster control plane to node kubelet and workloads"
  }
  ...
  ingress {
    ...
    description     = "Allow ALB to NodePort services"
  }
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
    description     = "Allow ALB to pod port 80"
  }
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
    description     = "Allow ALB to backend port 3000"
  }
...
resource "aws_security_group" "alb" {
  name        = "${var.project_name}-${var.environment}-eks-alb-sg"
...
resource "aws_eks_node_group" "main" {
  ...
  # Remote access for debugging (optional)
  # remote_access {
...
resource "aws_lb" "eks" {
  name               = "${var.project_name}-${var.environment}-eks-alb"
  internal           = false
...
# Target Group for Backend (NestJS)
...
resource "aws_lb_listener_rule" "backend_api" {
  ...
  condition {
    path_pattern {
      values = ["/api/*", "/query*", "/search/*", "/indexing/*", "/health"]
    }
  }
}

# Attach Node Group instances to Target Groups
